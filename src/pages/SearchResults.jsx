...
const SearchResults = () => {
  ...
  const { results, tab, searchType } = location.state || {
    results: [],
    tab: "Courses",
    searchType: null,
  };
  ...
  const destinations = useMemo(
    () => [...new Set(realResults.map((r) => r.destination).filter(Boolean))],
    [realResults],
  );
  const studyLevels = useMemo(() => {
    // Try studyLevel first, otherwise use coursesOffered
    const levels = realResults.flatMap((r) =>
      r.studyLevel ? [r.studyLevel] : r.coursesOffered || [],
    );
  ...
  const filteredResults = useMemo(() => {
    ...
    return actualResults
      .filter((r) => {
        ...
        if (
          filters.subject &&
          !(r.coursesOffered || []).includes(filters.subject)
        )
          return false;

        // Destination
        if (filters.destination && r.destination !== filters.destination)
          return false;

        // Study Level (adjust if studyLevel exists)
        if (filters.studyLevel && r.studyLevel !== filters.studyLevel)
          return false;
      ...
      .sort((a, b) => {
        switch (sortBy) {
          ...
          case "name":
            return (a.universityName || a.name || "").localeCompare(
              b.universityName || b.name || "",
            );
          case "fee": {
            const getFee = (r) => {
  ...
  const renderCourseCard = (course) => (
    <div
      key={course._id}
      className="bg-white rounded-lg mt-12 border border-gray-200 p-6 hover:shadow-md transition-all duration-200 h-full flex flex-col w-72"
    >
  ...
  const renderScholarshipCard = (scholarship) => (
    <div
      key={scholarship._id}
      className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-all duration-200 h-full flex flex-col"
    >
      <div className="flex justify-end mb-3">
        <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-medium">
      ...
      <div className="mb-4 flex-grow">
        <p className="text-gray-900 font-semibold text-sm mb-1">
          Study Level: {scholarship.studyLevel}
        </p>
        <p className="text-gray-900 font-semibold text-sm mb-3">
          Destination: {scholarship.destination}
        </p>
        <p className="text-gray-600 text-sm leading-relaxed">
          {scholarship.description ||
            `These ${scholarship.studyLevel} scholarships in ${scholarship.destination} are designed for students seeking comprehensive financial support for their education. Emphasis is placed on academic excellence, research potential, and global career readiness.`}
        </p>
      </div>
      <div className="flex gap-3 mt-auto">
        <Link
          to={`/search-results/scholarship-details/${scholarship?._id}`}
          className="flex-1 bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium"
        >
          View Details
        </Link>
  ...
  const renderUniversityCard = (university) => (
    <div
      key={university._id || university.universityName}
      className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 md:p-6 hover:shadow-md transition-all duration-200 h-full flex flex-col"
    >
      <div className="flex justify-between items-start mb-2 sm:mb-3">
        <div className="flex-1 min-w-0">
          <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 mb-1 truncate">
      ...
      <div className="mb-3 sm:mb-4 flex-grow">
        ...
        <div className="mb-2 sm:mb-3">
          ...
          <span className="text-green-600 font-semibold ml-1 text-xs">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-auto">
        <Link
          to={`/search-results/university-details/${university?._id}`}
  ...
  const renderEventCard = (event) => (
    ...
    >
      ...
      <div className="mb-4 flex-grow">
        ...
        <p className="text-gray-600 text-sm leading-relaxed">
          {event.description ||
            `Join our education fair in ${event.city || "major cities"} to explore study opportunities in ${event.destination}. Meet university representatives, learn about courses, scholarships, and application processes. Perfect for students planning their international education journey.`}
        </p>
      </div>
      <div className="flex gap-3 mt-auto">
        <Link
          to={`/search-results/event-details/${event?._id}`}
          className="flex-1 bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium"
        >
          View Details
        </Link>
  ...
  const renderResults = () => {
    ...
    if (!filteredResults || filteredResults.length === 0)
      return <div className="text-center py-8 sm:py-12">No results found</div>;
