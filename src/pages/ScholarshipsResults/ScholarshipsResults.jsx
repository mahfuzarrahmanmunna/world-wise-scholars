...
const ScholarshipsResults = () => {
  ...
  const destinations = useMemo(
    () => [...new Set(scholarships.map((s) => s.destination).filter(Boolean))],
    [scholarships],
  );
  const durations = useMemo(
    () => [...new Set(scholarships.map((s) => s.duration).filter(Boolean))],
    [scholarships],
  );
  const coursesIncludedList = useMemo(
    () => [
      ...new Set(
        scholarships.flatMap((s) => s.coursesIncluded).filter(Boolean),
      ),
    ],
    [scholarships],
  );
  ...
  const feeRange = useMemo(() => {
    const allFees = scholarships
      .map((s) => {
        ...
        return match
          ? match.map((n) => parseInt(n.replace(/[,€$SGD]/g, "")))
          : null;
      })
      .flat()
      .filter(Boolean);
    ...
    return scholarships
      .filter((s) => {
        ...
        if (filters.coursesIncluded) {
          const list = (s.coursesIncluded || []).map((c) =>
        ...
        if (filters.tuitionFee.length === 2) {
          ...
          if (match) {
            ...
            const maxFee = parseInt(
              match[match.length - 1].replace(/[,€$SGD]/g, ""),
            );
  ...
  const renderScholarshipCard = (scholarship) => (
    ...
    >
      ...
      <div className="mb-4 flex-grow">
        ...
        <p className="text-gray-900 font-semibold text-sm mb-1">
          Donation/Funding: {scholarship.tuitionFee}
        </p>
      ...
      <div className="flex gap-3 mt-auto">
        <Link
          to={`/search-results/scholarships/scholarship-details/${scholarship._id}`}
          ...
          <div className="w-full md:w-1/4 bg-white rounded-lg border border-gray-200 p-4">
            ...
            <div className="mb-4">
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("coursesIncluded", e.target.value)
                }
                className="border border-gray-300 rounded-lg px-2 py-1 w-full text-sm"
              >
                <option value="">All</option>
                {coursesIncludedList.map((c) => (
